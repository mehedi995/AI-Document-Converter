...
public class ErrorCategorizationTests : IDisposable
{
    ...
    public async Task Extract_PasswordProtectedPdf_ThrowsUnsupportedFileCategory_NotCorruptedDocument()
    {
        ...
        // not a fabricated header. Unlike the OOXML formats above, which are
        // detected from a bare OLE signature, the PDF path
        // (pdf_extractor._guard_openable) has to actually parse the document
        // with pdfminer to tell an encrypted file from a corrupt one -
        // pdfplumber reports both as the same exception type.
